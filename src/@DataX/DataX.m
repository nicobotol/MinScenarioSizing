classdef DataX < matlab.mixin.Copyable
  ...
  properties
      ...
      iniVecUnits {mustBeMember(iniVecUnits,{'MW','kW','m/s', 'W'})}= 'W'
      figControl = 0;
      bw_range; % bandwidth range for the optimization
  ...
  methods(Static = true)
    % static methods
      function EnergynScore = EnSco(obs,traj)
          ...
          %   This calculates the energy score given a set of generated scenario trajectories (traj) and the initial observations (obs)
          J  = size(traj,2); % generated trajectories
          K  = size(obs,2);  % initial observations
          s2 = 0;
          for i=1:J
              for j=1:J
      ...
      function [KL_metrics] = KLDiv(LoadedData, GeneratedData, BandWidth, n_zero_idx)
  
        KL_input = zeros(size(LoadedData, 1),1);
        KL_gen = zeros(size(LoadedData, 1),1);
       
        for i=1:size(n_zero_idx,2)
             
            % find the pdf of the input data and generated scenarios
            [pdfInputData] = ksdensity(LoadedData(n_zero_idx(i), :),LoadedData(n_zero_idx(i), :),  'Bandwidth', BandWidth, 'Support', [min(LoadedData(n_zero_idx(i), :))-1e-8, max(LoadedData(n_zero_idx(i), :))+1e-8]);
            pdfScenarios = ksdensity(GeneratedData(n_zero_idx(i), :), GeneratedData(n_zero_idx(i), :), 'Bandwidth', BandWidth, 'Support', [min(GeneratedData(n_zero_idx(i), :))-1e-8, max(GeneratedData(n_zero_idx(i), :))+1e-8]);
            
            % reorder the pdfs and eliminate duplicated values
            [x_input, idx] = unique(LoadedData(n_zero_idx(i), :));
            pdfInputData_unique = pdfInputData(idx);
            [x_gen, idx] = unique(GeneratedData(n_zero_idx(i), :));
            pdfScenarios_unique = pdfScenarios(idx);
            
            if (length(x_gen(2:end-1)) > 4) && (length(x_input(2:end-1)) > 4)
              % Determine tha range of the parameters
              max_min = max(min(LoadedData(n_zero_idx(i), :)), min(GeneratedData(n_zero_idx(i), :)));
              min_max = min(max(LoadedData(n_zero_idx(i), :)), max(GeneratedData(n_zero_idx(i), :)));
              pts = linspace(max_min, min_max, 1e3);
              % interpolate the pdfs to have them evaluated in the same points
              pdfInputDataInterp = interp1(x_input(2:end-1), pdfInputData_unique(2:end-1), pts, 'linear', 0);
              pdfScenariosInterp = interp1(x_gen(2:end-1), pdfScenarios_unique(2:end-1), pts, 'linear', 0);
  
              % remove the value corresponding to 0, to avoid numerical issues in the KL computation
              idx1 = find(pdfInputDataInterp >= 1e-8);
              pts_new = pts(idx1);
              pdfInputDataInterp_new = pdfInputDataInterp(idx1);
              pdfScenariosInterp_new = pdfScenariosInterp(idx1);
              idx2 = find(pdfScenariosInterp_new >= 1e-8);
              pts_new = pts_new(idx2);
              pdfInputDataInterp_new = pdfInputDataInterp_new(idx2);
              pdfScenariosInterp_new = pdfScenariosInterp_new(idx2);
  
              % Compute the Kullback-Leibler divergence
              KL_input(n_zero_idx(i)) = sum(pdfInputDataInterp_new.* log(pdfInputDataInterp_new ./ pdfScenariosInterp_new ));
              KL_gen(n_zero_idx(i)) = sum(pdfScenariosInterp_new  .* log(pdfScenariosInterp_new ./ pdfInputDataInterp_new));
            else 
              KL_input(n_zero_idx(i)) = 0;
              KL_gen(n_zero_idx(i)) = 0;
            end

        end

        KL_input_median = median(KL_input(n_zero_idx));
        KL_gen_median   = median(KL_gen(n_zero_idx));
        
        KL_input_mean   = mean(KL_input(n_zero_idx));
        KL_gen_mean     = mean(KL_gen(n_zero_idx));

        KL_input_std    = std(KL_input(n_zero_idx));
        KL_gen_std      = std(KL_gen(n_zero_idx));

        % integral absolute vale of the KL
        KL_input_int_error = sum(abs(KL_input(n_zero_idx)));
        KL_gen_int_error = sum(abs(KL_gen(n_zero_idx)));

        KL_metrics.KL_input             = KL_input;
        KL_metrics.KL_gen               = KL_gen;
        KL_metrics.KL_input_mean        = KL_input_mean;
        KL_metrics.KL_gen_mean          = KL_gen_mean;
        KL_metrics.KL_input_median      = KL_input_median;
        KL_metrics.KL_gen_median        = KL_gen_median;
        KL_metrics.KL_input_std         = KL_input_std;
        KL_metrics.KL_gen_std           = KL_gen_std;
        KL_metrics.KL_input_int_error   = KL_input_int_error;
        KL_metrics.KL_gen_int_error     = KL_gen_int_error;
...
%                         figure; hold on;
                      for i=1:size(objData.iniVec,1)
                        ...
                        % [~, ~, bw_opt] = kde(randVarX(:,i), "Bandwidth", "plug-in");
                        % if bw_opt <= 1e-3
                        %     bw_opt = 2;
                        % end
                        % uniformRandVarU(:,i)=ksdensity(randVarX(:,i),randVarX(:,i),'function','cdf','Bandwidth',bw_opt);

                        %   uniformRandVarU(:,i)=ksdensity(randVarX(:,i),randVarX(:,i),'function','cdf');
              ...
              if copulMethod == 1
                  ...
                  for i=1:size(objData.iniVec,1)
                    %   sampledRandVarXcopulG(:,i)=ksdensity(randVarX(:,i),copulGRandU(:,i),'function','icdf','Bandwidth',2);
                      ...
                      [~, ~, bw_opt] = kde(randVarX(:,i), "Bandwidth", "plug-in");
                      if bw_opt <= 1e-8; bw_opt = 1e-8; end
                      sampledRandVarXcopulG(:,i)=ksdensity(randVarX(:,i),copulGRandU(:,i),'function','icdf', 'Bandwidth', bw_opt);
...
%                         xlim([1 size(objData.iniVec,1)]);
    ...
    % ---Method 10 bis:
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=DoCopulaBW(objData,Nsamples,copulMethod,BandWidth)
        % DoCopulaBW Estimates a copula pdf from the input grouped data and samples from this Nsamples random scenarios, specifying of the bandwidth to use
        % copulMethod ([1]: T-copula| [2]: Gaussian)
        % Example: z4=test1.DoCopulaBW(10,2,5);
        % (test1 is the object)
        if size(objData.iniVec,2) == 1 || size(objData.iniVec,1) == 1
            disp('You gave ungrouped data and clustering requires grouped data');
            sampledRandVarXcopul = objData.iniVec;
            ...
            for i=1:size(objData.iniVec,1)
                uniformRandVarU(:,i)=ksdensity(randVarX(:,i),randVarX(:,i),'function','cdf','Bandwidth',BandWidth);
            end
            ...
            if copulMethod == 1
                ...
                % Transform the random sample back to the original scale of the data.
                for i=1:size(objData.iniVec,1)
                    sampledRandVarXcopulT(:,i)=ksdensity(randVarX(:,i),copulTRandU(:,i),'function','icdf','Support','positive','Bandwidth',BandWidth);
                end
                    sampledRandVarXcopul = sampledRandVarXcopulT';
                    R24 = Rho1;
                ...
                % Transform the random sample back to the original scale of the data.
                for i=1:size(objData.iniVec,1)
                    sampledRandVarXcopulG(:,i)=ksdensity(randVarX(:,i),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth);
                end
                ...
                %-----TEST BANDWIDTH----
                n_zero_idx = 1:1:size(objData.iniVec,1);
                inputdata = objData.iniVec;
                [KL_metrics] = DataX.KLDiv(inputdata, sampledRandVarXcopul, BandWidth, n_zero_idx);
                
        end
    end
    % ---Method 10 tris:
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL]=DoCopulaGauss(objData,Nsamples,copulMethod)
        % DoCopulaGauss Estimates a copula gaussian pdf from the input grouped data and samples from this Nsamples random scenarios
        % copulMethod ([1]: T-copula| [2]: Gaussian)
        % Example: z4=test1.DoCopulaGauss(10,2);
        % (test1 is the object)
        if size(objData.iniVec,2) == 1 || size(objData.iniVec,1) == 1
            disp('You gave ungrouped data and clustering requires grouped data');
            sampledRandVarXcopul = objData.iniVec;
            ...
            n_zero_idx = [];
            for i=1:size(objData.iniVec,1)
                mu(i) = mean(randVarX(:,i));
                sigma(i) = std(randVarX(:,i));
                if mu(i) < 1e-1
                    zero_idx = [zero_idx i];
                else
                    n_zero_idx = [n_zero_idx i];
                end
            end

            % Transform the data to the copula scale (unit square) using a kernel estimator of the cumulative distribution function.
            uniformRandVarU = zeros(size(objData.iniVec,2),length(n_zero_idx));

            for i=1:size(n_zero_idx,2)
                uniformRandVarU(:,i) = cdf('HalfNormal',randVarX(:,n_zero_idx(i) ), 1e-8,sigma(n_zero_idx(i)));
            end
            ...
            if copulMethod == 1
                ...
                copulGRandU = copularnd('Gaussian',Rho2,Nsamples);
                % Transform the random sample back to the original scale of the data.
                 sampledRandVarXcopulG = zeros(Nsamples,size(objData.iniVec,1));
                for i=1:size(n_zero_idx,2)
                    sampledRandVarXcopulG(:,n_zero_idx(i))=icdf('HalfNormal', copulGRandU(:,i), 0, sigma(n_zero_idx(i)));
                end
                ...
                %-----TEST BANDWIDTH----
                KL = zeros(size(objData.iniVec,1),1);
                for i=1:size(n_zero_idx,2)
                    pdfInputData = kde(objData.iniVec(n_zero_idx(i), :), "Bandwidth", "plug-in");

                    pdfScenarios = kde(sampledRandVarXcopul(n_zero_idx(i), :), "Bandwidth", "plug-in");

                    % Compute the Kullback-Leibler divergence
                    % KL(i) = sum(pdfInputData .* log(pdfInputData ./ pdfScenarios));
                    KL(n_zero_idx (i)) = sum(pdfScenarios .* log(pdfScenarios ./ pdfInputData));
    ...
    % ---Method 10 quater:
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=DoCopulaBWFilter(objData,Nsamples,copulMethod, BandWidth, threshold)
        % DoCopula Estimates a copula pdf from the input grouped data and samples from this Nsamples random scenarios, specifying of the bandwidth to use and filtering the initial dataset, such taht to consider only the value which are grather than a threshold
        % copulMethod ([1]: T-copula| [2]: Gaussian)
        % Example: z4=test1.DoCopulaBWFilter(10,2,5,1e-1);
        % (test1 is the object)
        if size(objData.iniVec,2) == 1 || size(objData.iniVec,1) == 1
            disp('You gave ungrouped data and clustering requires grouped data');
            sampledRandVarXcopul = objData.iniVec;
            ...
            % ---Estimate pdf of my multivariate input grouped data
            randVarX = objData.iniVec';
            % Find the timeseries of all 0
            zero_idx = [];
            n_zero_idx = [];
            for i=1:size(objData.iniVec,1)
                if sum(randVarX(:,i) > threshold) >= 1
                    n_zero_idx = [n_zero_idx i]; % index of the RV which are different from 0
                else
                    zero_idx = [zero_idx i]; % index of the RV which are 0
                end
            end
            ...
            for i=1:size(n_zero_idx,2)
                uniformRandVarU(:,i) = ksdensity(randVarX(:,n_zero_idx(i)),randVarX(:,n_zero_idx(i)),'function','cdf','Bandwidth',BandWidth);
            end
            ...
            if copulMethod == 1
                ...
                % Transform the random sample back to the original scale of the data.
                 sampledRandVarXcopulG = zeros(Nsamples,size(objData.iniVec,1));
                for i=1:size(n_zero_idx,2)
                    sampledRandVarXcopulG(:,n_zero_idx(i))=ksdensity(randVarX(:,n_zero_idx(i)),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth);
                end
      ...
      % ---Method 10 bis:
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=DoCopulaBWPositive(objData,Nsamples,copulMethod,BandWidth,threshold,uniqueSeed)
        % DoCopulaBW Estimates a copula pdf from the input grouped data and samples from this Nsamples random scenarios, specifying of the bandwidth to use
        % copulMethod ([1]: T-copula| [2]: Gaussian)
        % Example: z4=test1.DoCopulaBW(10,2,5);
        % (test1 is the object)
        ...
        if size(objData.iniVec,2) == 1 || size(objData.iniVec,1) == 1
            ...
            for i=1:size(n_zero_idx,2)
                uniformRandVarU(:,i)=ksdensity(randVarX(:,n_zero_idx(i)),randVarX(:,n_zero_idx(i)),'function','cdf','Bandwidth',BandWidth,'Support',[min(randVarX(:,n_zero_idx(i)))-1e-8,max(randVarX(:,n_zero_idx(i)))+1e-8]);
                % uniformRandVarU(:,i)=ksdensity(randVarX(:,n_zero_idx(i)),randVarX(:,n_zero_idx(i)),'function','cdf','Bandwidth',BandWidth,'Support',[-1e-8, 1e4]);
            end
            ...
            if copulMethod == 1
                ...
                for i=1:size(n_zero_idx,2)
                    sampledRandVarXcopulT(:,i)=ksdensity(randVarX(:,i),copulTRandU(:,i),'function','icdf','Support','positive','Bandwidth',BandWidth);
                    % sampledRandVarXcopulT(:,i)=ksdensity(randVarX(:,i),copulTRandU(:,i),'function','icdf','Support','positive','Bandwidth',BandWidth,'Support',[-1e-8, 1e4]);
                end
                    sampledRandVarXcopul = sampledRandVarXcopulT';
                    R24 = Rho1;
                ...
                sampledRandVarXcopulG = zeros(Nsamples,size(objData.iniVec,1));
                for i=1:size(n_zero_idx,2)
                    sampledRandVarXcopulG(:,n_zero_idx(i))=ksdensity(randVarX(:,n_zero_idx(i)),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth,'Support',[min(randVarX(:,n_zero_idx(i)))-1e-8, max(randVarX(:,n_zero_idx(i)))+1e-8]);
                    % sampledRandVarXcopulG(:,n_zero_idx(i))=ksdensity(randVarX(:,n_zero_idx(i)),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth,'Support',[-1e-8, 1e4]);
                end
    ...
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=DoCopulaBWPositive_par(objData,Nsamples,copulMethod,BandWidth,threshold,uniqueSeed)
        % DoCopulaBWPositive_par does the same as DoCopulaBWPositive but parallelizing the for loops 
        % copulMethod ([1]: T-copula| [2]: Gaussian)
        % Example: z4=test1.DoCopulaBW(10,2,5);
        % (test1 is the object)
        ...
        if size(objData.iniVec,2) == 1 || size(objData.iniVec,1) == 1
            ...
            zero_idx = [];
            n_zero_idx = [];
            parfor i=1:size(objData.iniVec,1)
                if sum(randVarX(:,i) > threshold) >= 1
                    n_zero_idx = [n_zero_idx i]; % index of the RV which are different from 0
                else
                    zero_idx = [zero_idx i]; % index of the RV which are 0
            ...
            if copulMethod == 1
                ...
                for i=1:size(n_zero_idx,2)
                    idx = n_zero_idx(i);
                    sampledRandVarXcopulG(:,idx)=ksdensity(randVarX(:,idx),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth,'Support',[min(randVarX(:,idx))-1e-8, max(randVarX(:,idx))+1e-8]);
                    % sampledRandVarXcopulG(:,n_zero_idx(i))=ksdensity(randVarX(:,n_zero_idx(i)),copulGRandU(:,i),'function','icdf','Bandwidth',BandWidth,'Support',[-1e-8, 1e4]);
                end
    ...
    % -- Method 10 quinquies:
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24,KL_metrics, opt_bw]=DoCopulaOptBW(objData,scenario,uniqueSeed)

        th = 1e-1; %treshold for consider value as 0

        % Find the optimal bandwidth
        [opt_bw] = objData.BandwithSelection(scenario, uniqueSeed, th);

        % Use the optimal bandwidth for estimate the copula
        [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=objData.DoCopulaBWPositive(scenario.InCopula,2,opt_bw,th, uniqueSeed);

    end

    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24,KL_metrics, opt_bw]=DoCopulaOptBW_par(objData,scenario,uniqueSeed)
        ...
        % Use the optimal bandwidth for estimate the copula
        [sampledRandVarXcopul,uniformRandVarU,EnSc,R24, KL_metrics]=objData.DoCopulaBWPositive_par(scenario.InCopula,2,opt_bw,th, uniqueSeed);

    end
    
    function [sampledRandVarXcopul,uniformRandVarU,EnSc,R24,KL_metrics, opt_bw]=DoCopulaOptBWPatternsearch(objData,scenario,uniqueSeed)

        th = 1e-1; %treshold for consider value as 0

        % Find the optimal bandwidth
        [opt_bw] = objData.BandwithSelectionPatternsearch(scenario, uniqueSeed, th);
      ...
      % ---Method 11:
            function [mappedCoords,closestPoints,RESselScens,LOADselScens,probVec,mapVars] = KantorMap2D(objDataRES,objDataLOAD,clustersNum,rngSeed)
            ...
            %   Example: [coords,points]=KantorMap2D(LoadC,DataX(WF1.Power));
      ...
      % ---Method 12:
      function [mappedCoords,closestPoints,RESselScens,LOADselScens,probVec,mapVars] = KantorMapND(cellObjDataRES,objDataLOAD,clustersNum, rngSeed)
          ...
          max_iter = 1;
          % ------------------ K-MEANS ELBOW PLOT -----------------------
          % Not implemented in the ND case yet
          %           Xobj=DataX(X);
          % %             Xobj.DoElbowPlot
          %           [mapVars.elbowPlotX,mapVars.elbowPlotY]= Xobj.DoElbowPlot;
        ...
        %   [idx,C] = kmeans(X,clustersNum);
          ...
          % The implementation of this part has been started but not finished yet
          % x_i = cell(numObj+1, 1);
          % for i = 1:numObj+1
          %   x_i{i} = min(X(:,i)):0.001:max(X(:,i));
          % end

          % x_iG = cell(numObj+1, 1);
          % [x_iG{:}] = ndgrid(x_i{:});

          % XGrid = [x_iG{:}]; % Defines a fine grid on the plot
      ...
      function [mappedCoords,closestPoints,RESselScens,LOADselScens,probVec,mapVars] = KantorMapND_par(cellObjDataRES,objDataLOAD,clustersNum, rngSeed)
          %KantorMapND_par works as KantorMapND but parallelizing the for loops
          %   This actually created the N-Dimension map with a ranked RES power
          %   variables on one axis and a platform's LOAD on the other
          %   Example: [coords,points]=KantorMapND(DataX(WF1.Power),LoadC);
          %   The RES are given as cell array
      ...
      %% ---Method 13: Select the bandwidth minimizing a parameter of the KL divergence
        function [opt_bw, KL] = BandwithSelection(objData, scenario, uniqueSeed, th)

            % set the algorithm for the optimization ot 'active-set' which seems to work better and faster than the default one
            options = optimoptions('fmincon', 'Algorithm', 'active-set','Display','none');
            % options = optimoptions('fmincon', 'Algorithm', 'sqp'); 
            % options = optimoptions('fmincon', 'Algorithm', 'interior-point');

            fun = @(bw)min_bw(objData, scenario.InCopula, bw, th, uniqueSeed);
            A = [];
            b = [];
            Aeq = [];
            beq = [];
            lb = objData.bw_range(1); % lower bound where to look for the optimal bandwidth
            ub = objData.bw_range(end); % upper bound where to look for the optimal bandwidth
            x0 = (lb + ub)/2; % initial guess for the optimal bandwidth
            [opt_bw, mean_min] = fmincon(fun, x0, A, b, Aeq, beq, lb, ub, [], options);
            % [opt_bw, mean_min] = patternsearch(fun, x0, A, b, Aeq, beq, lb, ub); # pattersearch works better in finding the minimum, but takes more time than fmincon

        end
        
        function [opt_bw, KL] = BandwithSelectionPatternsearch(objData, scenario, uniqueSeed, th)
        ...
        function y = min_bw(objData, num_scenarios, x, th, uniqueSeed)
            [~, ~, ES, ~, metric_struct] = objData.DoCopulaBWPositive(num_scenarios, 2, x, th, uniqueSeed);
            % y = abs(metric_struct.KL_input_int_error); % sum of abs of KL divergence
            y = ES; % energy score
        end
  ...
  %
    ...
    %         output = vectors of 0;
    %     else
    %         minimize_KL -> output = vector corresponding to optimal bandwidth
    %     end
    % end
