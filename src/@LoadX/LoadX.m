classdef LoadX < matlab.mixin.Copyable
  % LoadX class can perform the following operations on its objects:
  % 1) --Group
  % 2) --Ungroup
  % 3) --Interpolate
  % 4) --Average
  ...
  properties
      ...
      figControl = 0;
      delta_load    % percentage of load unbalance that can happen in one time step
      NS_cost;     % cost of not served power
      Pv_cost;     % cost of virtual power added
      CT_cost;     % cost of curtailed power
      LPSP_target; % Maximum LPSP
      NS_fraction; % fraction of hour load that can be not served 
      P_rated;     % rated power of the load
      P_shaved_frac;     % rated power of the load
      P_shaved_overpower; % factor of increase of load power w.r.t. the rated power when doing peak shaving
      Pps_add_cost;  % cost for adding power to the load within a scenario horizon
      Pps_rem_cost;  % cost for removing power to the load within a scenario horizon
      Pyr_add_cost;  % cost of adding power to the load within an optimization horizon
      Pyr_rem_cost;  % cost of removing power to the load within an optimization horizon
      Pct_max;      % Maximum amount of curtailed power
      % 		samples2Average
  end % properties
  % ---------------------------------------------------------------------
  methods
      % standard constructor
      function objData = LoadX(inputData)
...
%                         xlim([1 size(objData.iniVec,1)]);
      ...
      % ---Method 11:
            function [mappedCoords,closestPoints,RESselScens,...
                LOADselScens,probVec,mapVars] = KantorMap2D(objDataRES,objDataLOAD,clustersNum, rngSeed)
      ...
      % ---Method 12:
      function [mappedCoords,closestPoints,RESselScens,...
              LOADselScens,probVec,mapVars] = KantorMapND(cellObjDataRES,objDataLOAD,clustersNum, rngSeed)
